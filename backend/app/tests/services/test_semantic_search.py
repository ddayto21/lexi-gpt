...
from app.services.semantic_search import create_vector_embedding, calculate_similarity_scores, get_top_k_books
...
@pytest.mark.parametrize(
    "query,documents",
    [
        ("A science fiction novel", ["A futuristic space adventure", "A historical drama", "A book about AI"]),
        ("A detective mystery", ["A crime thriller", "A love story", "A biography of Sherlock Holmes"]),
        ("A book on machine learning", ["A deep learning guide", "A cooking recipe", "An autobiography"]),
...
def test_calculate_similarity_scores(query, documents, setup_fixture):
    ...
    document_embeddings = np.vstack([create_vector_embedding(model, doc, device) for doc in documents])

    # Compute similarity scores
    similarity_scores = calculate_similarity_scores(query_embedding, document_embeddings)

    # Assertions
    assert isinstance(similarity_scores, torch.Tensor), "Output should be a PyTorch tensor"
    assert similarity_scores.shape == (1, len(documents)), "Shape mismatch: Should be (1, num_documents)"
    ...
    assert (similarity_scores >= -1).all() and (similarity_scores <= 1).all(), "Similarity scores should be between -1 and 1"
...
def test_get_top_k_books(query, books_metadata, k, setup_fixture):
    ...
    book_embeddings = np.vstack(
        [create_vector_embedding(model, book["embedding_input"], device) for book in books_metadata]
    )
    ...
    assert all(isinstance(book, dict) for book in top_books), "Each item should be a dictionary"
