...
class ChatMessage(BaseModel):
    ...
    timestamp: Optional[datetime] = Field (None, description="Timestamp when the messsage was created.")
    parts: Optional[List[ChatPart]] = Field(default_factory = list, description = "List of message parts for detailed content breakdown.")]
    
class ChatRequest(BaseModel):
    """
    Represents the request body for the /chat endpoint.
    """
    id: Optional[str] = Field(None, description = "Unique identifier for the chat session.")
